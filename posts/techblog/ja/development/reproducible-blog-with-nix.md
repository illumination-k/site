...
`flake.nix` はPython 3.12系と [uv](https://docs.astral.sh/uv/) を提供するdevShellを定義している。
...
```nix title=flake.nix
{
  ...
  inputs = {
    # 2026-04 時点の nixos-25.11 の特定コミットに固定している。
    # flake.lock を生成すれば narHash レベルでも固定される。
...
companionディレクトリ内は `nix build` の `result` シンボリックリンクやdirenvのキャッシュで汚れるので、一通りignoreしておく。
...
逆に、記事本文とコードの二重管理はどうしても残る。記事に貼っているコードとcompanionディレクトリ内のコードが食い違う、というのは今のところ目視レビュー頼みで、自動化していない。本気で解決するには記事から `src/hello.py` を直接埋め込む仕組みが必要になり、そこまでやるとMDXのランタイムが絡んで重いので、費用対効果を見ながら判断する予定。
