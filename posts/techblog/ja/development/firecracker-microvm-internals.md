...
- BIOSもブートローダも持たず、VMMがカーネルイメージをゲストメモリへ直接展開し、zero pageやhvm_start_infoを組み立て、vCPUのレジスタを「64bitモードに入り終えた状態」に設定してから走らせる。これが起動時間125ms以下という数字の土台になっている
- I/Oの速さの本体はioeventfdとirqfdである。ゲストがvirtqueueへ通知を書くとKVMがカーネル内でeventfdを叩き、VMMスレッドのepollが起きる。KVM exitがユーザー空間まで戻ってこない
...
Firecrackerの設計を読むうえでの出発点は、[SPECIFICATION.md](https://github.com/firecracker-microvm/firecracker/blob/main/SPECIFICATION.md)に書かれた数値目標だ。これは努力目標ではなく、統合テストで毎PR検証される契約として扱われている。以下の3点はいずれも`m5d.metal`と`m6g.metal`のベアメタルインスタンス上での値になる。
...
この数字を守ろうとすると、汎用VMMが持っている機能のほとんどが邪魔になる。PCIバスの列挙、BIOS、SCSIやUSBのエミュレーション、VGA、ACPIの巨大なテーブル。これらはどれも起動時間とメモリフットプリントを食い、同時に攻撃面を広げる。Firecrackerの答えは単純で、それらを持たないことだ。
...
順序に意味がある。デバイスの生成がカーネルロードより後に来るのは、デバイスに割り当てたMMIOアドレスとIRQ番号をカーネルコマンドラインへ書き込む必要があるからだ。この点は後述する。
...
`MmioRead`と`MmioWrite`は`mmio_bus`へディスパッチされる。これはアドレス範囲をキーにデバイスを引く単純なバスで、virtioのMMIOトランスポート、シリアル、ブートタイマーなどがここに載っている。注意したいのは、この処理がVMMスレッドではなくvCPUスレッド自身の上で走る点だ。デバイスは`Arc<Mutex<...>>`で共有されており、vCPUスレッドがロックを取って設定レジスタを読み書きする。ここに来るのはvirtioデバイスの初期化やステータス更新のような低頻度のアクセスであり、データパスの通知は次節のioeventfdへ逃がされている。
...
ゲストメモリは、ホスト側で`mmap`した無名メモリをKVMのメモリスロットとして登録したものだ。

```rust
hugetlb_flags | libc::MAP_PRIVATE | libc::MAP_NORESERVE | libc::MAP_ANONYMOUS
```

`MAP_NORESERVE`が付いているのがポイントで、これによりホストのメモリを事前に確保しない。設計ドキュメントが言う「microVMはホストのCPUとメモリをオーバーサブスクライブできる」はこの一行に支えられている。実際にページが割り当てられるのは、ゲストが最初にそのページに触った時点になる。
...
PVHブートで使う`hvm_start_info`とLinuxブートプロトコルのzero pageが同じ`0x7000`付近に置かれているのは、この2つが排他だからだ。ソース中のコメントも「両者は排他なのでアドレスが重なってよい」と明記している。
...
::gh[https://github.com/firecracker-microvm/firecracker/blob/753a817/src/vmm/src/arch/x86_64/mod.rs#L450-L485]

まずELFとしてロードを試み、ELFマジックがなければbzImageローダにフォールバックする。つまり非圧縮の`vmlinux`とdistroが配る`bzImage`の両方を受け付ける。bzImageの場合は`XLF_KERNEL_64`フラグを確認したうえで、ロードアドレスから`0x200`進んだ64bitエントリポイントへ直接飛ぶ。リアルモードから始まる従来のブート経路はまるごと迂回される。
...
さらに`setup_sregs()`がセグメントレジスタと制御レジスタを構成する。Linuxブートプロトコルの場合は`CR0`に`PE`と`PG`、`CR4`に`PAE`、`EFER`に`LME`と`LMA`を立て、ゲストメモリ上に4段のページテーブルまで用意する。つまりvCPUは最初の1命令目から64bitロングモードで動き出す。PVHの場合はABIの規定どおりページング無効の32bitプロテクトモードで開始する。いずれにせよ、リアルモードから順にモードを切り替えていく通常のブートストラップは、KVMのioctl数回に置き換えられている。
...
x86_64のmicroVMがゲストへ見せるレガシーデバイスは、シリアルポートとi8042 (キーボードコントローラ) の2つに絞られる。しかもi8042は本来の用途では使われない。設計ドキュメントの表現を借りれば、Firecracker内でのi8042の目的は「ゲストがリブートを要求したことをmicroVMへ伝えること」だけだ。ホストから`SendCtrlAltDel`アクションを送ると、i8042がキーボード割り込みを注入し、ゲストのカーネルがそれをリブート要求として解釈する。
...
MMIOトランスポートでは1デバイスあたり`0x1000`バイトの領域と、レガシーGSI 1本が割り当てられる。x86_64のGSIは5番から23番までがレガシー用に予約されているので、この方式で載せられるvirtioデバイスの数には上限がある。PCIトランスポートを使う場合は`--enable-pci`フラグが必要で、有効にするとすべてのvirtioデバイスがPCI経由になりMSI-Xが使われる。デバイスのホットプラグもPCIトランスポートでのみ提供される (執筆時点ではdeveloper preview)。
...
I/Oの速さの本体はここにある。virtioデバイスを登録するコードを見ると、2つのeventfdがKVMへ渡されている。
...
重要なのは、通知の往路でKVM exitがユーザー空間まで戻らない点だ。vCPUスレッドは`KVM_RUN`の中に留まったまま別のゲストコードを実行し続け、I/Oの実処理は別スレッドが引き受ける。復路の割り込み注入も同様にカーネル内で完結する。VMMのプロセスコンテキストスイッチが往復で発生しないため、レイテンシとCPU消費の双方が下がる。
...
Firecracker側の判断は3点にまとめられている。MMDSを有効化していない場合そもそも`169.254.169.254`宛はTAPへ素通しであること、ゲストの送出トラフィックは信頼されない前提でホスト側のファイアウォールで制御すべきであること、そして修正のためにパケットをホストのバッファへコピーするとゲスト間TCPスループットが大きく落ちること。

これはFirecrackerの脅威モデルの引き方をよく表している。「ゲストの出力は信頼しない、フィルタリングはホストの責務」という境界が先にあり、その内側での局所的な競合は境界の外で対処されるべき問題として扱われている。
...
最も内側の境界はハードウェア仮想化そのものだ。ゲストのメモリアクセスはEPT/NPTで制限され、特権命令はVM exitを引き起こす。Firecrackerが自前で命令エミュレーションを持たないぶん、この境界を破るために必要なのはKVM自体の脆弱性になる。
...
| スレッド | 許可されるシステムコール数 | 既定動作 |
| -------- | -------------------------- | -------- |
| vmm      | 50                         | trap     |
| api      | 31                         | trap     |
| vcpu     | 27                         | trap     |

既定動作は`trap`、すなわち許可されていないシステムコールを呼ぶと`SIGSYS`でプロセスが落ちる。ルール数はシステムコール数より多い。これは同じシステムコールに対して引数の値まで含めた複数の条件が書かれているためで、たとえば`ioctl`はKVMの特定のリクエスト番号に限定される。
...
- `/proc/<pid>/fd`を走査し、標準入出力を除くファイルディスクリプタをすべて閉じる
- 親プロセスから受け継いだ環境変数をすべて消す
- `<chroot_base>/<exec_file_name>/<id>/root`を作り、Firecrackerバイナリをそこへコピーする。他のFirecrackerプロセスとメモリを共有させないためである
...
代償として、復元したmicroVMが生きている間はメモリファイルを消せない。またページフォルトのコストが実行中に分散して現れるため、レイテンシに敏感なワークロードでは`userfaultfd`を使った事前ロードが選択肢になる (公式ドキュメントに専用の解説がある)。
...
カーネルのfdtableは初期サイズが64で、超えると再確保が起きる。デバイスごとに複数のeventfdを持つ構成では簡単に超えるため、起動時に`dup2`と`close`を使って一度テーブルを最大サイズまで広げている。30msから70msという数字は、125msという全体予算の中では無視できない大きさだ。
...
なお、差分スナップショット (diff snapshot) は執筆時点でもdeveloper preview扱いになっている。ダーティページ追跡と`guest_memfd`対応の統合が検討中とされている。
