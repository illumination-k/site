...
第一正規形とは、全ての属性が**単一の値**を持つことです。具体的には、直積集合や冪集合を持たないことを指します。
...
`X = 伝票番号`、`Y = 顧客名`、`Z = 顧客番号` と定義すると、これは推移的関数従属となります。
...
第三正規形では更新時以上は発生しにくいですが、以下のような場合に起こりえます。
...
この分解によって、第三世紀形における更新時異常が解消されます。

### ボイスコッド正規形におけける関数従属損失
...
第四正規形では、**多値従属性**を排除します。

## 第五正規形 (5NF, The Fifth Normal Form)

第五正規形では、**結合従属性**を排除します。
