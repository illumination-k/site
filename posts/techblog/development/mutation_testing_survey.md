...
## 実践: このブログへのStrykerJS導入

このブログ（illumination-k.dev）自体にStrykerJSを導入し、PR単位で品質メトリクスを追跡する仕組みを構築した。現在の数値と構成を紹介する。

### 構成

このサイトはpnpm + Turboのモノレポで、mutation testingの対象は以下の3パッケージ。

- **md-plugins** — remarkプラグイン（見出しID付与、コードタイトル、GitHub埋め込み等）
- **ipynb2md** — Jupyter Notebookのmarkdown変換
- **cli** — 記事処理CLI（画像最適化、OG画像生成、RSS生成）

webパッケージ（Next.jsフロントエンド）はUIコンポーネント中心のためmutation testingの対象外とし、vitestのカバレッジのみ計測している。
...
Mutation Scoreが47%と低いのは、ipynb2mdパッケージ（30%）とcliパッケージのカバレッジ不足が主因。md-pluginsは62%で、remarkプラグインのテストが比較的充実している。

### CIへの組み込み

GitHub Actionsで以下のワークフローを構成した。

1. **PR時**: `pnpm test:coverage` + `stryker run` を実行し、結果をPRコメントとして自動投稿（既存コメントは上書き更新）
2. **mainマージ時**: 同じメトリクスを `metrics-history.json` に追記してコミット
...
- 初回のフル実行は約4分。CI上でPR単位で回すには許容範囲だが、プロジェクト規模が大きくなればincrementalモードが必須になる
- Mutation Score 47%は「テストがあるが質が低い」ことを明確に示している。カバレッジ70%だけ見ていると気付けない問題
